...
#else
void parse_option(int argc,char **argv,char* d,char** portp,char **logp)//信号解析
#endif
{
	int opt;
	static char port[16];
	...
	#else
	while((opt=getopt_long(argc,argv,":dp:l:hv",longopts,NULL))!=-1)//表示运行时的传入参数
		/*例如：
		  ./webd -d -p 8000    d表示守护进程p表示端口
		  ./webd -version 表示版本
		 */
	#endif
	{
