...
/* $begin sigaction */
handler_t *Signal(int signum, handler_t *handler) //信号处理
{
    struct sigaction action, old_action;

    action.sa_handler = handler;  //设置回调函数sa_handler统一处理
    sigemptyset(&action.sa_mask); /* block sigs of type being handled 设置屏蔽集*/
...
FILE *Fopen(const char *filename, const char *mode) 
{
    ...
    {
        syslog(LOG_CRIT,"Fopen %s error",filename);//写入日志
...
int open_listenfd(int port) 
{
    ...
    if ((listenfd = socket(AF_INET, SOCK_STREAM, 0)) < 0)//1.创建socket
	return -1;
...
//2.初始化地址
    ...
    if (bind(listenfd, (SA *)&serveraddr, sizeof(serveraddr)) < 0)//3.绑定socket
	return -1;

    /* Make it a listening socket ready to accept connection requests */
    if (listen(listenfd, LISTENQ) < 0)//4.监听
	return -1;
    return listenfd;
}
/* $end open_listenfd */
