...
int socket_accept(int st)
{
	...
	while (1)//因为http是基于tcp协议的，所以socket的创建和tcp 的一致
	{
		...
		} else
		{
			...
			*tmp = client_st;
			pthread_create(&thrd_t, &attr, http_thread, tmp);//这里面创建线程去执行回调函数http_thread(),参数为&client_st
		}
...
int  get_file_content(const char *file_name/*url路径*/, char **content/* *content为要读入的内存buff */) // 得到文件内容
{
	int  file_length = 0;
	FILE *fp = NULL;
	...
	fp = fopen(file_name, "rb");//以可读可写的形式打开文件，file_name为相对路径
	...
	fseek(fp, 0, SEEK_END);//将文件读写指针放在最后，方便ftell大小
	file_length = ftell(fp);//ftell查询当前文件的大小，把文件读到内存，把内存写入socket，封装成HTTP响应，发过去
	rewind(fp);//将文件读写指针指向文件开头
	...
	*content = (char *) malloc(file_length);
	...
	fread(*content, file_length, 1, fp);//将fp的文件读入*content（buff中）
	fclose(fp);
//从中可以看到二级指针char **content 目的是用来返回*content的，因为*content是malloc创建的堆内存
//客户将url发给服务器---服务器解析url，取到相对路径--根据路径读取本地网页文件到内存buff--封装为http响应--写入socket
